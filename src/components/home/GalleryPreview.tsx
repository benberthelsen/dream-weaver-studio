...
export function GalleryPreview() {
  ...
  const filteredItems = useMemo(
    () => (activeFilter === "All" ? galleryItems : galleryItems.filter((item) => item.room === activeFilter)).slice(0, 6),
    [activeFilter]
  );
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-10">
          {filters.map((filter) => (
            <Button
              ...
              asChild
            >
              <Link to={filter === "All" ? "/gallery" : `/gallery?room=${encodeURIComponent(filter)}`}>{filter}</Link>
            </Button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-6xl mx-auto">
          {filteredItems.map((item) => (
            <Link key={item.title} to={`/gallery?room=${encodeURIComponent(item.room)}`} className="group relative aspect-[4/3] overflow-hidden rounded-lg bg-muted">
              <img
                src={item.image}
                alt={item.title}
